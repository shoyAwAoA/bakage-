...
//グローバル変数
bool silverback_Speak_flag = false;
bool silverback_DieSpeak_flag = false;
...
void SceneGame3::Initialize()
{
	...
	silverbackSpeak = new Sprite("Data/Sprite/riza.png");//最初のセリフ
	silverbackDieSpeak = new Sprite("Data/Sprite/suku3.png");//倒された時のシルバーバック先輩のセリフ
	...
	for (int i = 0; i < 1; i++)
	{
		EnemySilverback* Silver = new EnemySilverback();
		Silver->SetPosition(DirectX::XMFLOAT3(i * 2.0f, 0, 5));
		Silver->SetTerritory(Silver->GetPosition(), 10.0f);
		enemyManager.Register(Silver);
	}
#endif
}
...
void SceneGame3::Finalize()
{
	...
	if (silverbackSpeak != nullptr)
	{
		delete silverbackSpeak;
		silverbackSpeak = nullptr;
	}
	if (silverbackDieSpeak != nullptr)
	{
		delete silverbackDieSpeak;
		silverbackDieSpeak = nullptr;
	}
	if (spr != nullptr)
	{
		delete spr;
...
//エネミーHPゲージ描画
void SceneGame3::RenderEnemyGauge(
	...
	const DirectX::XMFLOAT4X4& projection)
{
	...
	for (int i = 0; i < enemyCount; ++i)
	{
		...
		if (silverback_Speak_flag)
		{
			silverbackSpeak->Render(dc,
				screenPosition.x,
				screenPosition.y + 20,
				speakWidth,
				speakHeight,
				0, 0,
				static_cast<float>(silverbackSpeak->GetTextureWidth()),
				static_cast<float>(silverbackSpeak->GetTextureHeight()),
				0.0f,
				1.0f, 1.0f, 1.0f, 1.0f
			);
		}
		else if (silverback_DieSpeak_flag)
		{
			silverbackDieSpeak->Render(dc,
				dieSpeakPositionX,
				dieSpeakPositionY,
				dieSpeakWidth,
				dieSpeakHeight,
				0, 0,
				static_cast<float>(silverbackDieSpeak->GetTextureWidth()),
				static_cast<float>(silverbackDieSpeak->GetTextureHeight()),
				0.0f,
				1.0f, 1.0f, 1.0f, 1.0f
			);
		}
