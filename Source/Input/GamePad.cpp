...
void GamePad::Update()
{
	...
	if (XInputGetState(slot, &xinputState) == ERROR_SUCCESS)
	{
		...
		if (pad.wButtons & XINPUT_GAMEPAD_A)						newButtonState |= BTN_Z;
		if (pad.wButtons & XINPUT_GAMEPAD_B)						newButtonState |= BTN_X;
		if (pad.wButtons & XINPUT_GAMEPAD_X)						newButtonState |= BTN_C;
		if (pad.wButtons & XINPUT_GAMEPAD_Y)						newButtonState |= BTN_V;
	...
	// キーボードでエミュレーション
	{
		...
		if (GetAsyncKeyState('X') & 0x8000) newButtonState |= BTN_X;
		if (GetAsyncKeyState('C') & 0x8000) newButtonState |= BTN_C;
		if (GetAsyncKeyState('V') & 0x8000) newButtonState |= BTN_V;
		...
		if (GetAsyncKeyState('D') & 0x8000) newButtonState |= BTN_D;
